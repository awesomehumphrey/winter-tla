...
\caption{The coordinated split view showing the Die Hard water jugs specification. Left: clustered force-directed graph with six clusters (one per big-jug value), color-coded by cluster membership, with convex hulls around clusters with two or more members. Right: adjacency matrix with cluster-based ordering, showing transitions as filled cells and cluster boundaries as colored blocks along the diagonal.}
...
\textbf{Visual encoding.} Nodes are rendered as circles colored by cluster membership using D3's categorical color scale. Each node displays a text label showing its state description (e.g., \texttt{big=3,small=0}). Edges between nodes are rendered as paths; for pairs of nodes with bidirectional transitions, the default view shows a single slightly thicker line to reduce clutter, while unidirectional edges use a thinner line. Convex hulls are drawn around clusters with two or more members using a semi-transparent fill of the cluster color, providing a visual boundary that reinforces the cluster grouping.

\textbf{Interaction.} Hovering over a node triggers a progressive disclosure interaction: the selected node and its immediate neighbors remain at full opacity, while all other nodes fade to 20\% opacity. Connected edges turn red and display directional arrowheads; for bidirectional pairs, the single default line splits into two curved edges with arrowheads showing each direction. Unconnected edges fade. A tooltip displays the state label, cluster identifier, and in/out degree. This interaction directly supports task T3 (transition inspection) by isolating the selected state's connectivity from the visual clutter of the full graph, while revealing directional detail only when actively inspecting. When the user moves the mouse away from the node, the full graph is restored.

\textbf{Cluster-level aggregation.} For clusters with two or more members, a centroid circle is drawn at the cluster center, providing a visual anchor for the cluster. Aggregate cluster-level edges connect clusters with curved arcs whose thickness encodes the number of individual transitions between those clusters. Clicking a cluster-level edge reveals the individual node-to-node edges between the two clusters, enabling drill-down from the overview level to the detail level. This progressive disclosure approach follows Shneiderman's visual information seeking mantra~\cite{shneiderman1996eyes}: the cluster-level edges provide the overview, while clicking reveals the details on demand.
...
\textbf{Graph view (T1, T2).} The clustered force-directed layout spatially separates the six clusters, with each cluster rendered in a distinct color. Clusters with two or more members are enclosed in convex hulls, providing clear visual boundaries for all six clusters. Aggregate cluster-level edges connect cluster centroids with curved arcs whose thickness encodes the number of individual transitions between clusters, giving an immediate sense of inter-cluster connectivity density. The engineer can observe (T1) that the state space has a non-trivial structure with multiple clusters of varying sizes, and (T2) identify which clusters correspond to which big-jug values. The two largest clusters (big=0 and big=5) are at opposite ends of the variable's range, each containing states where the small jug holds different amounts of water. Clicking a cluster-level edge reveals the individual node-to-node transitions between those two clusters, enabling drill-down from the overview. Hovering over a node highlights its connected edges in red and reveals directional arrowheads; for bidirectional transitions, the single default line splits into two curved edges showing each direction.
...
\textbf{Matrix view (T1, T2, T3).} The adjacency matrix, ordered by cluster membership, reveals a rich block structure. Intra-cluster transitions appear as dense blocks along the diagonal---for example, the \texttt{big=5} cluster (4 states) forms a $4 \times 4$ block in the upper-left corner, while the \texttt{big=0} cluster (4 states) forms a corresponding block in the lower-right corner. Filled cells within these blocks represent transitions between states that share the same big-jug value but differ in small-jug value. Inter-cluster transitions form off-diagonal patterns that reveal the connectivity between clusters.

The matrix makes several structural properties immediately visible that would be difficult to discern in the graph view: (1) most states have four outgoing transitions (four filled cells per row), but boundary states---where one or both jugs are completely full or empty---have fewer (two or three), reflecting that actions such as \texttt{FillBig} on an already-full jug or \texttt{EmptySmall} on an empty jug are disabled; (2) the transition pattern is not symmetric---the matrix is not symmetric about the diagonal, reflecting the directed nature of the state transitions; (3) certain cluster pairs have dense connectivity (e.g., transitions between \texttt{big=0} and \texttt{big=5} are common because \texttt{FillBig} from any state with \texttt{big=0} reaches \texttt{big=5}), while others have sparse or no connectivity.

\textbf{Coordinated view (T3, T4).} The coordinated view enables comprehensive analysis of this complex specification. Consider an engineer investigating how to reach the goal state \texttt{big=4,small=0}---the solution to the Die Hard puzzle. The engineer can locate this state in the graph view (in the \texttt{big=4} cluster) and hover over it, causing the matrix view to highlight the corresponding row and column. The highlighted column shows predecessor states with transitions \textit{into} the goal state, while the highlighted row shows states reachable \textit{from} the goal state.

By inspecting the predecessor states in the matrix and moving backward step by step---hovering over each predecessor to reveal its own incoming transitions---the engineer can incrementally trace a path from the initial state to the goal. At each step, the graph view provides spatial context, showing where each state sits relative to its cluster, while the matrix view provides precise connectivity information. This interleaved, step-by-step exploration supports task T4 (path tracing) and demonstrates the value of coordinated views for complex specifications.
...
\textbf{Edge bundling.} Our current design uses aggregate cluster-level edges to summarise inter-cluster connectivity, with individual edges revealed on demand. When these individual edges are displayed, they can still produce visual clutter for dense cluster pairs. Hierarchical edge bundling~\cite{holten2006hierarchical} could reduce this clutter by routing revealed edges along the cluster hierarchy, grouping edges that share source or target clusters into smooth bundles rather than independent lines.
