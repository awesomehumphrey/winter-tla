...
\begin{abstract}
Formal specifications written in TLA+ enable rigorous verification of mission-critical systems through exhaustive model checking. Organizations including Amazon Web Services and Microsoft have adopted TLA+ to verify the correctness of complex distributed systems and safety-critical middleware. However, the state spaces produced by the TLC model checker are difficult for engineers to comprehend: even moderately complex specifications generate hundreds or thousands of states and transitions. Previous visualization tools render these state spaces as standard force-directed node-link diagrams, which degenerate into illegible tangles beyond a few dozen states---a well-documented problem in the graph visualization literature. We present a design study of a web-based multi-view visualization tool that addresses this scalability challenge through three coordinated representations: (1) a clustered force-directed graph view that groups states sharing common variable values, with convex hull boundaries and interactive detail-on-demand; (2) an adjacency matrix view with cluster-based ordering that reveals connectivity patterns at higher information density than node-link diagrams; and (3) a synchronized split view enabling bidirectional cross-referencing between representations via a brushing-and-linking interaction protocol. We describe our design process, including a task analysis derived from deploying TLA+ specifications in industry projects, a data characterization of TLC model checker output, and design rationale for our visualization choices informed by the complementary strengths of graph and matrix representations. We demonstrate the tool through detailed usage scenarios on three TLA+ specifications of increasing complexity---HourClock (12 states, cyclic), a distributed surveillance system (3 states, multi-action), and the Die Hard water jugs problem (16 states, 58 transitions)---and show how the multi-view approach enables engineers to understand state structure, verify transitions, trace execution paths, and navigate specifications that would be illegible in a single-view node-link diagram.
\end{abstract}
...
Formal system specification provides significant advantages for mission-critical systems engineering. Specifications written in formal languages are concise, precise, and amenable to automated verification through model checking and theorem proving. Among formal specification languages, TLA+ (Temporal Logic of Actions)~\cite{lamport2003specifying, lamport2009pluscal} has emerged as one of the most widely adopted in industry. Amazon Web Services uses TLA+ to verify the correctness of complex distributed systems including DynamoDB, S3, and EBS~\cite{newcombe2015amazon}, while Microsoft has applied TLA+ to the development of safety-critical fault-tolerant middleware~\cite{resch2017tla}. The TLC model checker~\cite{yuan1999model} enables engineers to exhaustively enumerate all reachable states and transitions for a given specification, providing confidence that the system behaves correctly under all possible scenarios.

However, TLA+ specifications require deep expertise in temporal logic, and the state spaces they produce are inherently difficult to comprehend. Each state in a TLA+ specification represents a complete assignment of values to all specification variables, and each transition corresponds to an action that transforms one state into another. Even for moderately complex specifications, the number of reachable states grows combinatorially with the number of variables and their domains---a phenomenon known as the state explosion problem~\cite{liu2005model}. For example, a specification with ten variables each capable of holding ten distinct values has a theoretical state space of $10^{10}$ states. While the TLC model checker can enumerate these states efficiently, presenting the results to engineers in a comprehensible manner remains an open challenge.

The current state of TLA+ visualization is limited. The TLA+ Toolbox provides textual output from the TLC model checker, listing states and transitions in a sequential format that becomes overwhelming for large specifications. As an initial step toward addressing this gap, we developed a prototype tool that renders TLA+ state spaces as interactive state machine diagrams using a force-directed layout, supporting stepping through states and visual debugging. However, this prototype used a standard force-directed layout without clustering or alternative representations, and its visualizations degenerated into illegible ``hairball'' diagrams beyond a few dozen states (Fig.~\ref{fig:hairball}). This scalability limitation is well-documented in the graph visualization literature: force-directed layouts produce visual clutter at scale due to edge crossings, node overlaps, and the difficulty of perceiving structure in dense graphs~\cite{harel1988visual, herman2000graph}. Diehl~\cite{diehl2007softvis} provides a comprehensive treatment of software visualization techniques, but existing approaches focus primarily on source code structure and runtime behavior rather than formal specification state spaces.
...
For TLA+ specifically, the TLA+ Toolbox provides the TLC model checker with textual output, but does not include graphical visualization of the state space. We previously developed a prototype tool that renders TLA+ state spaces as interactive state machine diagrams. This prototype parses the TLA+ model file, runs the TLC model checker to extract states and transitions, and renders the result as a force-directed graph. The tool supports visual debugging features including stepping over states, selecting next states to visit, and highlighting the connection between specification elements and visualization elements. However, the prototype's single-view force-directed layout lacks clustering, and its scalability is limited: for specifications with more than approximately 20 states, the visualization degenerates into a ``hairball'' with overlapping nodes and crossing edges that obscure the structure of the state space (see Fig.~\ref{fig:hairball}).
...
Users load a TLA+ state space by uploading a JSON file through a file input control. In the split view, the uploaded file is forwarded to both embedded views via \texttt{postMessage}, ensuring that both views display the same data. A clear button resets both views simultaneously. Navigation links in the toolbar allow users to switch between the three views.
...
The surveillance system specification models a simplified distributed surveillance system derived from an industry project. The system can be in one of three states---\textit{UNKNOWN}, \textit{OK}, and \textit{FAILED}---and transitions between them via two actions: \texttt{report\_received} (indicating a sensor message was received) and \texttt{timeout} (indicating no message was received within a time window). The state space consists of 3 states, each in its own cluster (since the only variable is \texttt{state}, and each state has a unique value), connected by 4 transitions.
...
The Die Hard water jugs problem is a well-known combinatorial puzzle that models two water jugs with capacities of 5 gallons (big) and 3 gallons (small), with six possible actions: \texttt{FillBig}, \texttt{FillSmall}, \texttt{EmptyBig}, \texttt{EmptySmall}, \texttt{BigToSmall} (pour from big to small), and \texttt{SmallToBig} (pour from small to big). The state space consists of 16 reachable states and 58 transitions---already dense enough to produce significant visual clutter in a flat node-link diagram. States are grouped by the value of the \texttt{big} variable into six clusters: \texttt{big=0} (4 states), \texttt{big=1} (2 states), \texttt{big=2} (2 states), \texttt{big=3} (2 states), \texttt{big=4} (2 states), and \texttt{big=5} (4 states).
...
Our earlier prototype was deployed in industry projects that employ TLA+-based event model specifications. Engineers who previously struggled to understand the TLA+ domain-specific language found the single-view visualization and animation features greatly assisted their understanding and use of the specifications. Several engineers reported that the visual debugger helped them identify and correct complex errors in specifications that would have been difficult to find through textual inspection alone. However, engineers also noted that the force-directed layout became unreadable for specifications with more than approximately 20 states, directly motivating the multi-view approach presented in this paper.

The multi-view tool described in this paper has been developed iteratively, with design decisions informed by the feedback gathered during the prototype's deployment. However, we have not yet conducted a formal user evaluation of the multi-view tool itself. The usage scenarios presented in Section~\ref{sec:scenarios} are based on our own expert analysis rather than observed user behavior. A formal evaluation is a priority for future work, as discussed below.
...
\bibitem{lamport2003specifying}
L. Lamport, \textit{Specifying Systems: The TLA+ Language and Tools for Hardware and Software Engineers}. Boston, MA: Addison-Wesley, 2003.

\bibitem{lamport2009pluscal}
L. Lamport, ``The PlusCal algorithm language,'' in \textit{Proc. Int. Colloquium on Theoretical Aspects of Computing (ICTAC)}, pp. 36--60, Springer, 2009.

\bibitem{newcombe2015amazon}
C. Newcombe, T. Rath, F. Zhang, B. Munteanu, M. Brooker, and M. Deardeuff, ``How Amazon Web Services uses formal methods,'' \textit{Commun. ACM}, vol.~58, no.~4, pp.~66--73, 2015.

\bibitem{resch2017tla}
S. Resch and M. Paulitsch, ``Using TLA+ in the development of a safety-critical fault-tolerant middleware,'' in \textit{Proc. IEEE Int. Symp. Software Reliability Engineering Workshops (ISSREW)}, Toulouse, France, pp.~146--152, IEEE, 2017.

\bibitem{yuan1999model}
Y. Yu, P. Manolios, and L. Lamport, ``Model checking TLA+ specifications,'' in \textit{Proc. IFIP WG 10.5 Advanced Research Working Conf. on Correct Hardware Design and Verification Methods (CHARME '99)}, London, UK, pp.~54--66, Springer-Verlag, 1999.

\bibitem{wongsuphasawat2018tensorflow}
K. Wongsuphasawat, D. Smilkov, J. Wexler, J. Wilson, D. Man\'{e}, D. Fritz, D. Krishnan, F. B. Vi\'{e}gas, and M. Wattenberg, ``Visualizing dataflow graphs of deep learning models in TensorFlow,'' \textit{IEEE Trans. Vis. Comput. Graph.}, vol.~24, no.~1, pp.~1--12, 2018.

\bibitem{harel1988visual}
D. Harel, ``On visual formalisms,'' \textit{Commun. ACM}, vol.~31, no.~5, pp.~514--530, 1988.

\bibitem{harel1987statecharts}
D. Harel, ``Statecharts: A visual formalism for complex systems,'' \textit{Sci. Comput. Program.}, vol.~8, no.~3, pp.~231--274, 1987.

\bibitem{liu2005model}
X. Liu et~al., ``Using model checker and replay facility to debug complex distributed system,'' in \textit{Proc. 20th ACM Symp. Operating Systems Principles (SOSP)}, ACM, 2005.

\bibitem{ghoniem2004comparison}
M. Ghoniem, J.-D. Fekete, and P. Castagliola, ``A comparison of the readability of graphs using node-link and matrix-based representations,'' in \textit{Proc. IEEE Symp. Information Visualization (InfoVis)}, Austin, TX, USA, pp.~17--24, 2004.

\bibitem{henry2006matrixexplorer}
N. Henry and J.-D. Fekete, ``MatrixExplorer: A dual-representation system to explore social networks,'' \textit{IEEE Trans. Vis. Comput. Graph.}, vol.~12, no.~5, pp.~677--684, 2006.

\bibitem{henry2007nodetrix}
N. Henry, J.-D. Fekete, and M. J. McGuffin, ``NodeTrix: A hybrid visualization of social networks,'' \textit{IEEE Trans. Vis. Comput. Graph.}, vol.~13, no.~6, pp.~1302--1309, 2007.

\bibitem{henry2007matlink}
N. Henry and J.-D. Fekete, ``MatLink: Enhanced matrix visualization for analyzing social networks,'' in \textit{Proc. INTERACT}, pp.~288--302, Springer, 2007.

\bibitem{herman2000graph}
I. Herman, G. Melan\c{c}on, and M. S. Marshall, ``Graph visualization and navigation in information visualization: A survey,'' \textit{IEEE Trans. Vis. Comput. Graph.}, vol.~6, no.~1, pp.~24--43, 2000.

\bibitem{dunne2013motif}
C. Dunne and B. Shneiderman, ``Motif simplification: Improving network visualization readability with fan, connector, and clique glyphs,'' in \textit{Proc. ACM SIGCHI Conf. Human Factors in Computing Systems (CHI)}, pp.~3247--3256, ACM, 2013.

\bibitem{holten2006hierarchical}
D. Holten, ``Hierarchical edge bundles: Visualization of adjacency relations in hierarchical data,'' \textit{IEEE Trans. Vis. Comput. Graph.}, vol.~12, no.~5, pp.~741--748, 2006.

\bibitem{bertin1983semiology}
J. Bertin, \textit{Semiology of Graphics: Diagrams, Networks, Maps}. Madison, WI: University of Wisconsin Press, 1983.

\bibitem{behrisch2016reordering}
M. Behrisch, B. Bach, N. Henry Riche, T. Schreck, and J.-D. Fekete, ``Matrix reordering methods for table and network visualization,'' \textit{Computer Graphics Forum}, vol.~35, no.~3, pp.~693--716, 2016.

\bibitem{eades1996hierarchical}
P. Eades, Q.-W. Feng, and X. Lin, ``Straight-line drawing algorithms for hierarchical graphs and clustered graphs,'' in \textit{Proc. Graph Drawing (GD)}, pp.~113--128, Springer, 1996.

\bibitem{sugiyama1981hierarchical}
K. Sugiyama, S. Tagawa, and M. Toda, ``Methods for visual understanding of hierarchical system structures,'' \textit{IEEE Trans. Syst., Man, Cybern.}, vol.~11, no.~2, pp.~109--125, 1981.

\bibitem{shneiderman1996eyes}
B. Shneiderman, ``The eyes have it: A task by data type taxonomy for information visualizations,'' in \textit{Proc. IEEE Symp. Visual Languages}, Boulder, CO, USA, pp.~336--343, 1996.

\bibitem{roberts2007cmv}
J. C. Roberts, ``State of the art: Coordinated \& multiple views in exploratory visualization,'' in \textit{Proc. 5th Int. Conf. Coordinated and Multiple Views in Exploratory Visualization (CMV)}, Zurich, Switzerland, pp.~61--71, 2007.

\bibitem{north2000snap}
C. North and B. Shneiderman, ``Snap-together visualization: A user study on coordinated views using tight coupling through operations and data,'' \textit{Int. J. Human-Computer Studies}, vol.~53, no.~5, pp.~715--739, 2000.

\bibitem{baldonado2000guidelines}
M. Q. Wang Baldonado, A. Woodruff, and A. Kuchinsky, ``Guidelines for using multiple views in information visualization,'' in \textit{Proc. Working Conf. Advanced Visual Interfaces (AVI)}, Palermo, Italy, pp.~110--119, 2000.

\bibitem{card1999readings}
S. K. Card, J. D. Mackinlay, and B. Shneiderman, \textit{Readings in Information Visualization: Using Vision to Think}. San Francisco, CA: Morgan Kaufmann, 1999.

\bibitem{munzner2009nested}
T. Munzner, ``A nested model for visualization design and validation,'' \textit{IEEE Trans. Vis. Comput. Graph.}, vol.~15, no.~6, pp.~921--928, 2009.

\bibitem{sedlmair2012design}
M. Sedlmair, M. Meyer, and T. Munzner, ``Design study methodology: Reflections from the trenches and the stacks,'' \textit{IEEE Trans. Vis. Comput. Graph.}, vol.~18, no.~12, pp.~2431--2440, 2012.
...
D. Jackson, \textit{Software Abstractions: Logic, Language, and Analysis}, revised ed. Cambridge, MA: MIT Press, 2012.

\bibitem{leuschel2008prob}
M. Leuschel and M. Butler, ``ProB: An automated analysis toolset for the B method,'' \textit{Int. J. Software Tools for Technology Transfer}, vol.~10, no.~2, pp.~185--203, 2008.

\bibitem{abrial1996bbook}
J.-R. Abrial, \textit{The B-Book: Assigning Programs to Meanings}. Cambridge, UK: Cambridge University Press, 1996.

\bibitem{behrmann2004uppaal}
G. Behrmann, A. David, and K. G. Larsen, ``A tutorial on UPPAAL,'' in \textit{Formal Methods for the Design of Real-Time Systems (SFM-RT 2004)}, Lecture Notes in Computer Science, vol.~3185, pp.~200--236, Springer, 2004.

\bibitem{abrial2010rodin}
J.-R. Abrial, M. Butler, S. Hallerstede, T. S. Hoang, F. Mehta, and L. Voisin, ``Rodin: An open toolset for modelling and reasoning in Event-B,'' \textit{Int. J. Software Tools for Technology Transfer}, vol.~12, no.~6, pp.~447--466, 2010.

\bibitem{diehl2007softvis}
S. Diehl, \textit{Software Visualization: Visualizing the Structure, Behaviour, and Evolution of Software}. Berlin: Springer, 2007.
