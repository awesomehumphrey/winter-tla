...
The HourClock specification~\cite{lamport2003specifying} models a clock cycling through values 1 to 12 via a single action \texttt{HCnxt}. The state space consists of 12 states in a single cluster connected in a cycle. The graph view displays all 12 nodes in a single cluster within a convex hull. The matrix view reveals the cyclic structure with much greater clarity: a diagonal band of filled cells one step above the main diagonal, plus a single cell at position $(12,1)$ representing the wrap-around transition. An engineer can verify at a glance that the specification is deterministic and complete. In the split view, hovering over a specific state (e.g., state \texttt{hr=6}) immediately highlights the corresponding row and column in the matrix, confirming that state 6 has exactly one incoming transition (from state 5) and one outgoing transition (to state 7). \textbf{Insight.} For this regular, single-cluster specification, the matrix view is more informative than the graph view, supporting the finding of Ghoniem et al.~\cite{ghoniem2004comparison} that matrix representations are superior for tasks involving precise connectivity assessment.
...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{figures/splitview_diehard.png}
