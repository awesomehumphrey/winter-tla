...
An evaluation following two complementary tracks, qualitative feedback from engineers working with TLA+ in industry and a controlled user study comparing task performance across views, is our priority for future work. Additional future directions include direct integration with the TLC model checker to streamline the workflow, temporal exploration through animated execution traces that highlight the current state and advance through a sequence of actions (supporting a more dynamic form of path tracing than the current static graph permits), hierarchical edge bundling~\cite{holten2006hierarchical} to reduce visual clutter for dense cluster pairs, and bidirectional editing that propagates changes from the visualisation back to the TLA+ source.
